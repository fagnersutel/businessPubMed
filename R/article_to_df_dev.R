...
function (pubmedArticle, autofill = FALSE, max_chars = 500) 
{
  ...
  tmp.article <- custom_grep(xml_data = pubmedArticle, tag = "PubmedArticle", 
                             format = "char")
  tmp.article <- ifelse(length(tmp.article) > 0, yes = tmp.article, NA)
  tmp.title <- custom_grep(xml_data = tmp.article, tag = "ArticleTitle", 
                           format = "char")
  tmp.title <- ifelse(length(tmp.title) > 0, yes = tmp.title, NA)
  ...
  sel.dateType <- ifelse(length(sel.dateType) > 0, yes = sel.dateType, no = 1)
  ...
  if(length(tmp.date) > 0){
    tmp.date <- sapply(c("Year", "Month", "Day"), (function(tt) {
      tmp.grep.data <- custom_grep(xml_data = tmp.date, tag = tt, format = "char")
      ifelse(length(tmp.grep.data)>0, tmp.grep.data, NA)
    }))
  } else {
    tmp.date <- c(NA, NA, NA)
  }
  tmp.paperID <- custom_grep(xml_data = tmp.article, tag = "ArticleIdList", 
                               format = "char")
  #
  tmp.paperID.2 <- regexpr('ELocationID[[:space:]]EIdType=[[:punct:]]doi[[:punct:]].{10,100}<\\/ELocationID>',
                            tmp.article)
  if (tmp.paperID.2 > 0) {
    tmp.paperID.2 <- substr(tmp.article,
                            tmp.paperID.2, 
                            tmp.paperID.2 + attributes(tmp.paperID.2)$match.length - 2)
    
    tmp.paperID.2 <- gsub ("^.*>", "", gsub ("<.*$", "", tmp.paperID.2))
    
    tmp.paperID.2 <- gsub("EIdType.+>", ">", tmp.paperID.2)
    tmp.paperID.2 <- paste('ArticleIdIdType=\"doi\">', 
                           tmp.paperID.2, 
                           '</ArticleId>')
    tmp.paperID <- paste(c(tmp.paperID, tmp.paperID.2), collapse = " ")
  }
  tmp.paperID <- ifelse(length(tmp.paperID)>0, tmp.paperID, "")
  #
  tmp.paperID <- gsub("[[:space:]]", "", tmp.paperID)
  tmp.PMID <- gsub("^(.*ArticleIdIdType=\\\"pubmed\\\")([[:space:]]|[[:alnum:]]){0,30}>", 
                   "", tmp.paperID)
  tmp.PMID <- gsub("<.*$", "", tmp.PMID)
  tmp.PMID <- ifelse (length(tmp.PMID) > 0, tmp.PMID, NA)
  tmp.DOI <- gsub("^(.*ArticleIdIdType=\\\"doi\\\")([[:space:]]|[[:alnum:]]){0,30}>", 
                  "", tmp.paperID)
  tmp.DOI <- gsub("<.*$", "", tmp.DOI)
  tmp.DOI <- ifelse (length(tmp.DOI) > 0, tmp.DOI, NA)
  #
  tmp.jabbrv <- custom_grep(xml_data = tmp.article, tag = "ISOAbbreviation", 
                            format = "char")
  tmp.jabbrv <- ifelse (length(tmp.jabbrv) > 0, tmp.jabbrv, NA)
  #
  tmp.journal <- custom_grep(xml_data = tmp.article, tag = "Title", 
                             format = "char")
  tmp.journal <- ifelse (length(tmp.journal) > 0, tmp.journal, NA)
  #
  tmp.resout <- c(pmid = tmp.PMID, doi = tmp.DOI, title = tmp.title, 
                  abstract = substr(tmp.abstract, 0, max_chars), year = as.character(tmp.date[1]), 
                  month = as.character(tmp.date[2]), day = as.character(tmp.date[3]), 
                  jabbrv = tmp.jabbrv, journal = tmp.journal)
  ...
  if (length(author.list) > 0){
    final.mat <- t(sapply(author.list, (function(al) {
      tmp.lastnm <- custom_grep(xml_data = al, tag = "LastName", 
                                format = "char")
      tmp.lastnm <- ifelse (length(tmp.lastnm) > 0, tmp.lastnm[1], NA)
      #
      tmp.firstnm <- custom_grep(xml_data = al, tag = "ForeName", 
                                 format = "char")
      tmp.firstnm <- ifelse (length(tmp.firstnm) > 0, tmp.firstnm[1], NA)
      tmp.email <- regexpr("([[:alnum:]]|\\.|\\-\\_){3,200}@([[:alnum:]]|\\.|\\-\\_){3,200}(\\.)([[:alnum:]]){2,6}", 
                           al)
      if (tmp.email > 0) {
        tmp.email <- substr(al, tmp.email, tmp.email + attributes(tmp.email)$match.length - 
                              1)
      } else {
        tmp.email <- NA
      }
    ...
    final.mat <- final.mat[!is.na(final.mat$lastname),]
    if (autofill) {
